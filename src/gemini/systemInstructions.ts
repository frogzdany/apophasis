// Lucy's per-language system prompt. Lives in its own module so the
// server-side ephemeral-token mint can bake it into liveConnectConstraints —
// when those constraints are present, the Live API drops anything the
// browser-side config tries to add (the same reason voice was getting
// defaulted before we moved speechConfig server-side).
