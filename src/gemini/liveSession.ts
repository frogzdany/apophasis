...
UI orchestration (use these to build / refine the panel):
  ...
  - open_drawing_canvas({ message }) — opens a drawing canvas so the user
    can sketch what they are looking for. The AI will interpret the sketch
    and send you a [drawing_context] message; use it immediately to search
    or render_surface. Trigger this when:
      • User says "let me draw it", "draw for me", "I'll sketch it"
      • User expresses you're not understanding: "you're not getting it",
        "that's not what I mean", "no me estás entendiendo"
      • After 2+ failed attempts to identify the same thing
...
UI (para construir / refinar el panel):
  ...
  - open_drawing_canvas({ message }) — abre un canvas de dibujo para que el
    usuario dibuje lo que busca. La IA interpretará el boceto y te enviará
    un mensaje [drawing_context]; úsalo de inmediato para buscar o renderizar
    un panel. Actívalo cuando:
      • El usuario diga "te lo dibujo", "déjame dibujarlo", "quiero dibujarte"
      • El usuario exprese que no lo estás entendiendo: "no me estás
        entendiendo", "no me comprendes", "eso no es lo que busco"
      • Tras 2+ intentos fallidos de identificar lo mismo
