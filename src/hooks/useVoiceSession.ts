...
export function useVoiceSession() {
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      // When the DrawingCanvas finishes interpretation it dispatches
      // 'lucyDrawingContext' with the AI-generated description. We forward
      // it to the live session so Lucy can act on the visual context.
      const onDrawingContext = (e: Event) => {
        ...
        const text = [
          ...
          'Use this visual context immediately — call search_* with the most fitting',
          'provider, or render_surface with fields pre-filled from the description.',
        ].join('\n')
